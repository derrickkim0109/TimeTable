...
final class LectureDetailView: BaseView {
    ...
    func makeMemoViews(_ memos: [MemoEntity]) {
        for (index, memo) in memos.enumerated() {
            ...
            memoStackView.addArrangedSubview(makeStackView(memoTitleView,
                                                           deleteButton))
    ...
    @objc private func didTapDeleteButton(_ sender: UIButton) {
        if let viewController = self.findViewController() as? LectureDetailViewController {
            guard let memos = viewController.memoModel else {
                return
            }

            viewController.memoModel?.remove(at: sender.tag)
            viewController.didDeleteMemo(memos[sender.tag])
        }
    }
}
